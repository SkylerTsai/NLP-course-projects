...
class DataCollatorWithPadding:
    ...
    def __call__(self, features):
        _id = [feature.pop('ids') for feature in features]
...
def main():
    ...
    labels = [label for label in raw_datasets['train'].features.keys() if label not in [
                'id',
                'review',
    ...
    model, optimizer, train_dataloader, eval_dataloader, lr_scheduler, predict_dataloader = accelerator.prepare(
        model, optimizer, train_dataloader, eval_dataloader, lr_scheduler, predict_dataloader
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            predictions[np.where(probs < 0.45)] = 0
            if step == 0:
                y_pred = torch.Tensor([predictions]).int()
                y_true = batch[0]["labels"].int().cpu()
            else:
                y_pred = torch.cat((y_pred, torch.Tensor([predictions]).int()), 0)
                y_true = torch.cat((y_true, batch[0]["labels"].int().cpu()), 0)


        # finally, compute metrics
        f1_micro_average = f1_score(y_true=y_true, y_pred=y_pred, average='micro')
        roc_auc = roc_auc_score(y_true, y_pred, average = 'micro')
        accuracy = accuracy_score(y_true.reshape(-1, 1), y_pred.reshape(-1, 1))
        precision = precision_score(y_true.reshape(-1, 1), y_pred.reshape(-1, 1))
        recall = recall_score(y_true.reshape(-1, 1), y_pred.reshape(-1, 1))
    ...
    for step, batch in enumerate(predict_dataloader):
        ...
        sigmoid = torch.nn.Sigmoid()
        probs = sigmoid(outputs.logits.squeeze().cpu())
        # print(outputs.logits.squeeze().cpu())
        prediction = np.zeros(probs.shape)
        prediction[np.where(probs >= 0.45)] = 1
        # prediction = outputs.logits.squeeze().cpu()
        # prediction[np.where(prediction >= 0.5)] = 1
        if prediction.size == 18:
    ...
    output_predict_file = os.path.join(args.output_dir, f"prediction.csv")
    with open(output_predict_file, "w") as csvfile:
        logger.info(f"***** Predict results *****")
        writer = csv.writer(csvfile)
        writer.writerow(["id-#aspect", "predicted"])
        for index, item in enumerate(predictions):
            ans = item.tolist()
            for i in range(18):
                if ans[i] == 1.0:
                    writer.writerow([str(_ids[index])+'-'+str(i+1), _ids[index]])
                else:
                    writer.writerow([str(_ids[index])+'-'+str(i+1), None])
